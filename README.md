...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/jma_resolve_area \
  -H 'Content-Type: application/json' \
  -d '{"query":"Tokyo"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/jma_resolve_area`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
